...
def view_note_from_db(user):
    ...
    with get_connection() as conn:
        cursor = conn.cursor()
        cursor.execute("SELECT id, note, tags, due_date FROM notes WHERE user = ?", (user,))
...
def import_txt_to_db(user):
    ...
    with open(notes_file, "r") as file:
        for line in file:
            ...
            note_part = " ".join([w for w in line.split() if not w.startswith("#") and not w.startswith("[due:")])
            due_date = None
            if "[due:" in line:
                try:
...
def filter_notes_by_tag(user, tag):
    ...
    with get_connection() as conn:
        cursor = conn.cursor()
        query = "SELECT id, note, tags, due_date FROM notes WHERE user = ? AND tags LIKE ?"
...
def edit_note_by_id(user, note_id):
    ...
    with get_connection() as conn:
        cursor = conn.cursor()
        cursor.execute("SELECT note, tags, due_date FROM notes WHERE id = ? AND user = ?", (note_id, user))
        row = cursor.fetchone()
...
def db_note_exists(user, note_id):
    ...
    cursor = conn.cursor()
    cursor.execute(
        "SELECT 1 FROM notes WHERE id = ? AND user = ?",
            (note_id, user)
    )
    ...
    while True:
        ...
        if choice == "1":
            ...
            if len(your_note) > 500:
                console.print("[red]Note too long (max 500 characters). Please shorten it.[/red]")
                continue
